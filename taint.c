...
static void php_taint_fcall_check(zend_execute_data *ex, const zend_op *opline, zend_function *fbc) /* {{{ */ {
    ...
    if (fbc->common.scope == NULL) {
        ...
        if (zend_string_equals_literal(fname, "print_r")) {
            ...
            if (arg_count > 1) {
                ...
                if (IS_STRING == Z_TYPE_P(fp)) {
                    ...
                    }else if (TAINT_POSSIBLE(Z_STR_P(fp))){
                        php_taint_error(ZSTR_VAL(fname), "Attempt to write file which filename might be tainted");
                    }
        ...
        } else if (zend_string_equals_literal(fname, "json_decode")) {
            /* TODO: allow_classes? */
            zval *str = ZEND_CALL_ARG(ex, 1);
            if (IS_STRING == Z_TYPE_P(str) && TAINT_POSSIBLE(Z_STR_P(str))) {
                php_taint_error(ZSTR_VAL(fname), "Attempt to json_decode a string that might be tainted");
            }
